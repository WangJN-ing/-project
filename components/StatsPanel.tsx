...
const StatItem = ({ label, value, unit, icon, colorClass = "text-sciblue-500" }: any) => (
  <div className="flex flex-col bg-white/60 backdrop-blur-md p-4 rounded-lg border border-slate-200 shadow-sm relative group overflow-hidden hover:shadow-md transition-shadow">
    ...
    <div className="flex items-center gap-2 text-slate-600 mb-1 text-[10px] uppercase tracking-widest font-bold font-mono">
      <span className={colorClass}>{React.cloneElement(icon, { size: 12 })}</span> {label}
    </div>
    
    <div className="text-xl md:text-2xl font-mono text-slate-900 font-bold tracking-tight flex items-baseline gap-1 mt-auto">
      {value.toFixed(3)} 
  ...
  return (
    ...
    >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 w-full pt-4">
           <StatItem label={t.stats.temperature} value={stats.temperature} unit="K" icon={<Thermometer/>} colorClass="text-rose-500"/>
           <StatItem label={t.stats.pressure} value={stats.pressure} unit="Pa" icon={<Gauge/>} colorClass="text-violet-500"/>
           <StatItem label={t.stats.meanSpeed} value={stats.meanSpeed} unit="m/s" icon={<Wind/>} colorClass="text-sciblue-500"/>
           <StatItem label={t.stats.rmsSpeed} value={stats.rmsSpeed} unit="m/s" icon={<Activity/>} colorClass="text-emerald-500"/>
           ...
           <div className="col-span-2 md:col-span-4 mt-2 px-1">
             ...
             {/* Progress Track */}
             <div className="w-full bg-slate-200 rounded-sm h-1.5 overflow-hidden border border-slate-300/50">
                <div 
